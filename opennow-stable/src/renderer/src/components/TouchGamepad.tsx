 ...
 * TouchGamepad.tsx
 *
 * Virtual on-screen gamepad for Android touch devices.
 * Renders: D-pad, dual thumbsticks, face buttons (A/B/X/Y),
 * shoulder buttons (LB/RB/LT/RT), and Start/Back.
 *
...
function FaceButton({ label, color, xinputFlag, clientRef }: FaceButtonProps): JSX.Element {
  ...
  return (
    <button
      ...
      style={{ borderColor: color, color: pressed ? "#000" : color, background: pressed ? color : "rgba(0,0,0,0.5)" }}
...
function Dpad({ clientRef }: DpadProps): JSX.Element {
  ...
  const releaseAll = useCallback(() => {
    for (const flag of pressed.current) clientRef.current?.sendGamepadButton(flag, false);
    pressed.current.clear();
  }, [clientRef]);

  const flagsAt = (el: HTMLElement, cx: number, cy: number): number[] => {
  ...
  return (
    <div
      className="tgp-dpad"
      onTouchStart={(e) => { e.preventDefault(); for (const t of Array.from(e.changedTouches)) for (const f of flagsAt(e.currentTarget, t.clientX, t.clientY)) press(f); }}
      onTouchMove={(e) => {
        e.preventDefault();
        const next = new Set<number>();
        for (const t of Array.from(e.touches)) for (const f of flagsAt(e.currentTarget, t.clientX, t.clientY)) next.add(f);
        for (const f of pressed.current) if (!next.has(f)) release(f);
        for (const f of next) press(f);
      }}
      onTouchEnd={(e) => { e.preventDefault(); if (e.touches.length === 0) releaseAll(); }}
      onTouchCancel={(e) => { e.preventDefault(); releaseAll(); }}
    >
...
function Thumbstick({ side, clientRef }: ThumbstickProps): JSX.Element {
  ...
  return (
    <div
      ref={stickRef}
      className="tgp-stick"
      onTouchStart={(e) => { e.preventDefault(); if (activeId.current !== null) return; const t = e.changedTouches[0]; if (t) activeId.current = t.identifier; }}
      onTouchMove={(e) => {
        e.preventDefault();
        const base = stickRef.current;
        if (!base || activeId.current === null) return;
        let touch: Touch | undefined;
        for (const t of Array.from(e.touches)) if (t.identifier === activeId.current) { touch = t; break; }
        if (!touch) return;
        ...
        setOffset({ x: dx, y: dy });
        clientRef.current?.sendGamepadStick(side, clamp(dx / RADIUS), clamp(dy / RADIUS));
      }}
      onTouchEnd={(e) => {
        e.preventDefault();
        let found = false;
        for (const t of Array.from(e.changedTouches)) if (t.identifier === activeId.current) { found = true; break; }
        if (!found) return;
        activeId.current = null; setOffset({ x: 0, y: 0 }); clientRef.current?.sendGamepadStick(side, 0, 0);
      }}
      onTouchCancel={(e) => { e.preventDefault(); activeId.current = null; setOffset({ x: 0, y: 0 }); clientRef.current?.sendGamepadStick(side, 0, 0); }}
    >
      <div className="tgp-stick-knob" style={{ transform: `translate(${offset.x}px, ${offset.y}px)` }} />
    </div>
  );
}

// --- Shoulder / Centre Buttons ---

function ShoulderButton({ label, xinputFlag, clientRef }: { label: string; xinputFlag: number; clientRef: React.RefObject<GfnWebRtcClient | null> }): JSX.Element {
  const [pressed, setPressed] = useState(false);
  return (
    <button
      type="button"
...
function CentreButton({ label, xinputFlag, clientRef }: { label: string; xinputFlag: number; clientRef: React.RefObject<GfnWebRtcClient | null> }): JSX.Element {
  const [pressed, setPressed] = useState(false);
  return (
    <button
      type="button"
